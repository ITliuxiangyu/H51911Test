...
$(".tabbar_item").click(function () {
    ...
    $(".app_content>div").each(function (index , item) {
        if ($(that).index() == index) {
            var cur_src = $(".tabbar_item").eq(index).find("img").attr("src")
            $(".tabbar_item").find("img").attr("src" , cur_src.replace(".png" , "_a.png"))
            $(item).css({
                "display": "block"
            })
        } else {
            var cur_src = $(".tabbar_item").eq(index).find("img").attr("src")
            console.log(cur_src)
            $(".tabbar_item").find("img").attr("src" , cur_src.replace("_a" , ""))
